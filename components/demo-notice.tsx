    ...
    return (
      <div className="relative border-b border-orange-200 bg-orange-50">
        <p className="mx-auto w-full max-w-6xl px-10 py-2 text-center text-xs leading-5 text-zinc-700 sm:px-14 sm:text-sm">
          ...
          demonstração. O OpenReply é auto-hospedado: entrar aqui não vai enviar
          DMs pela sua conta.{" "}
          <a
            href={SETUP_DOCS_URL}
            target="_blank"
  ...
  return (
    <div className="relative mb-5 rounded border border-warning/30 bg-warning/10 px-4 py-3 pr-10">
      <p className="text-sm leading-6 text-foreground">
        <span className="font-semibold">{DEMO_HOST} é uma instância de demonstração.</span>{" "}
        Entrar aqui não vai enviar DMs pela sua conta do Instagram. O OpenReply
        é auto-hospedado, então só funciona em uma instalação que você mesmo
        mantém, com seu próprio app da Meta e seu próprio domínio.{" "}
        <a
          href={SETUP_DOCS_URL}
          target="_blank"
          rel="noreferrer"
